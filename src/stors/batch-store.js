...
export const useBatchStore = defineStore("batch", {
  ...
  getters: {
    ...
    getProcessedBatches(state) {
      return state.batches.map((batch) => {
        const {
          name, code, program, batch_supervisor, current_level, num_group, start_date, end_date, status, ...rest
        } = batch;

        const currentStatusId = status ?? null;
        const foundStatus = state.statuses.find(st => String(st.id) === String(currentStatusId));
        ...
        return {
          ...
          program: program || null,
          batch_supervisor: batch_supervisor || null,
          current_level: current_level || null, // تبدأ فارغة أو تأتي من السيرفر كـ ID
          num_group: num_group || 0,
          status: currentStatusId,
          status_name: foundStatus ? foundStatus.name : "نشطة",
          formattedStartDate: start_date ? DefaultVal.ArabicDate(start_date) : "غير محدد",
          formattedEndDate: end_date ? DefaultVal.ArabicDate(end_date) : "غير محدد",
  ...
  actions: {
    async getBatchStoreData() {
      ...
      try {
        ...
        console.error("خطأ جلب الدفعات بالتوازي:", err);
      } finally {
        this.loading = false;
      }
    },
    ...
    async getBatchStatuses() {
      try {
        const { url, method } = endpoint.batches.getStatus;
        const { data } = await requestData(url, method);
        if (data) {
          this.statuses = Array.isArray(data) ? data : (data.results || []);
        } else {
          this.statuses = [{ id: 1, name: "نشطة" }, { id: 2, name: "مكتملة" }, { id: 3, name: "مؤجلة" }];
        }
      } catch (err) {
        this.statuses = [{ id: 1, name: "نشطة" }, { id: 2, name: "مكتملة" }, { id: 3, name: "مؤجلة" }];
      }
    ...
    async createBatch(batchData) {
      try {
        const { url, method } = endpoint.batches.createBatch;
        const { data, error } = await requestData(url, method, batchData);
        if (data) {
    ...
    async updateBatch(batchId, batchData) {
      try {
        const { url, method } = endpoint.batches.updateBatch;
        const finalUrl = `${url}${batchId}/`; 
        const { data, error } = await requestData(finalUrl, method, batchData);
        if (data) {
          const index = this.batches.findIndex((b) => b.id === batchId);
          if (index !== -1) this.batches[index] = data;
          return { success: true, data };
        }
        return { success: false, message: error };
      } catch (err) {
